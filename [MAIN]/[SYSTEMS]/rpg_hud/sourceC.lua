...
addEventHandler('onClientKey', root,
	function(key, state) 
		if (state) then
			if (key == 'F11') then
				cancelEvent();
				if (not getElementData(localPlayer,"p:info")) then return end
...
function getMapFromWorldPosition(worldX, worldY)
	local mapX = (Bigmap.PosX + Bigmap.Width / 2) + ((worldX - playerX) * Bigmap.CurrentZoom) * Minimap.MapUnit;
	local mapY = (Bigmap.PosY + Bigmap.Height / 2) - ((worldY - playerY) * Bigmap.CurrentZoom) * Minimap.MapUnit;
	
	return mapX, mapY;
end

function getWorldFromMapPosition(mapX, mapY)
	local worldX = playerX + ((mapX - (Bigmap.PosX + Bigmap.Width / 2)) / Bigmap.CurrentZoom) / Minimap.MapUnit;
	local worldY = playerY - ((mapY - (Bigmap.PosY + Bigmap.Height / 2)) / Bigmap.CurrentZoom) / Minimap.MapUnit;
